	...
	<div class="app-topbar">
		<div class="app-topbar-top">
			<div>
				...
				<m-tooltip class="mvi-ml-8" title="刷新" trigger="hover"><m-icon @click="refresh" type="refresh-a" class="mvi-cursor-pointer mvi-font-h6" /></m-tooltip>
				<div class="app-topbar-search mvi-ml-8">
					<m-field
						:active-color="$store.getters.theme.themeColor"
						round
						size="small"
						v-model.trim="search"
						placeholder="全局搜索..."
						clearable
						:prefix="{ type: 'search', size: '.3rem' }"
					></m-field>
				</div>
			</div>
			<div>
				<m-tooltip title="消息中心" trigger="hover"><m-icon type="bell-alt" class="mvi-cursor-pointer mvi-font-h6" /></m-tooltip>
				<div id="noteRoot" class="mvi-ml-8" style="position: relative;">
					<m-tooltip id="noteTarget" title="便签" trigger="hover"><m-icon @click="openNote" type="sale-alt" class="mvi-cursor-pointer mvi-font-h6" /></m-tooltip>
					<m-layer target="#noteTarget" root="#noteRoot" v-model="noteShow" placement="bottom-end" offset="0.2rem" :z-index="400" ref="noteLayer" width="6rem" closable>
						<div
							class="app-topbar-note-header"
							:style="{ color: $store.getters.theme.topbarNoteHeaderColor, backgroundColor: $store.getters.theme.topbarNoteHeaderBackground }"
						>
							<span>便签</span>
					...
					<m-layer target="#adminTarget" root="#adminRoot" v-model="infoShow" placement="bottom" offset="0.1rem" :z-index="400" border ref="adminLayer">
						<div class="app-topbar-info">
							...
							<div class="app-topbar-info-btn">
								<m-button
									@click="doLogout"
									:color="$store.getters.theme.themeColor"
									:sub-color="$store.getters.theme.themeTextColor"
									plain
									form-control
									size="mini"
									class="mvi-m-0"
				...
				<m-tooltip title="换肤" class="mvi-ml-4" trigger="hover">
					<div
						@click="skinPopup = true"
						class="app-topbar-skin"
						:style="{ color: $store.getters.theme.themeTextColor, backgroundColor: $store.getters.theme.themeColor }"
					>
				...
				<div class="mvi-mb-4">
					<m-input id="color-6" label="便签头部背景" label-width="1.6rem" v-model.trim="theme.topbarNoteHeaderBackground" border>
						<template v-slot:right>
							<div
								class="app-skin-demo"
								@click="topbarNoteHeaderBackgroundShow = !topbarNoteHeaderBackgroundShow"
								:style="{ backgroundColor: theme.topbarNoteHeaderBackground }"
							></div>
						</template>
					</m-input>
				...
				<div class="mvi-mb-4">
					<m-input id="color-7" label="便签头部字体" label-width="1.6rem" v-model.trim="theme.topbarNoteHeaderColor" border>
						<template v-slot:right>
							<div
								class="app-skin-demo"
								@click="topbarNoteHeaderColorShow = !topbarNoteHeaderColorShow"
								:style="{ backgroundColor: theme.topbarNoteHeaderColor }"
							></div>
						</template>
					</m-input>
